...
export interface RunPropertiesOptions {
  ...
  bold?: boolean;
  italic?: boolean;
  underline?: true | UnderlineType;
  // EG_TextUnderlineLine
  underlineLine?: { followText?: true } & LineOptions;
  // EG_TextUnderlineFill
  underlineFill?: { followText?: true } & FillOptions;
  strike?: true | StrikeType;
  kerning?: STPoint;
  cap?: CapType;
  charSpacing?: STPoint;
  ...
  // EG_EffectProperties
  effect?: {
    list?: EffectListOptions[];
    dag?: EffectContainerOptions;
  };
...
export class RunProperties extends XmlComponent {
  ...
  xmlComponent (): XmlElement {
    ...
    const attr = {
      ...
      b: options.bold,
      i: options.italic,
      u: typeof options.underline === 'string'
        ? options.underline
        : (options.underline ? StrikeType.Single : undefined),
      strike: typeof options.strike === 'string'
        ? options.strike
        : (options.strike ? StrikeType.Single : undefined),
      kern: toHpt(options.kerning),
      cap: options.cap,
      spc: toHpt(options.charSpacing),
      normalizeH: options.normalizeHeights,
      baseline: toPct(options.baseline)
        ?? (options.subscript ? -40000 : (options.superscript ? 30000 : undefined)),
    ...
    if (options.line) {
      children.push(new Line(options.line))
    }
    if (options.fill) {
      children.push(new Fill(options.fill))
    }
    if (options.effect?.list) {
      children.push(new EffectList(options.effect.list))
    } else if (options.effect?.dag) {
      children.push(new EffectDag(options.effect.dag))
    }
    if (options.highlight) {
      children.push({
        tag: 'a:highlight',
        children: [new Color(options.highlight)],
      })
    }
    if (options.underlineLine?.followText === true) {
      children.push({ tag: 'a:uLnTx' })
    } else if (options.underlineLine) {
      children.push(new UnderlineLine(options.underlineLine))
    }
    if (options.underlineFill?.followText === true) {
      children.push({ tag: 'a:uFillTx' })
    } else if (options.underlineFill) {
      children.push({
        tag: 'a:uFill',
        children: [new Fill(options.underlineFill)],
      })
    }
    if (options.font) {
      children.push(new Font(options.font))
    }
    if (options.click) {
      children.push(new HyperlinkClick(options.click))
    }
    if (options.mouseHover) {
      children.push(new HyperlinkMouseOver(options.mouseHover))
    }
